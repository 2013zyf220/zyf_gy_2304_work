...
rc_ana_3_mean <- function(f_grid_num, f_breaks_2_end, f_breaks_2_by){
  ...
  #f_file_name1 <- paste('shp/3/ppa_2301_rce_3_mean_', order_1, '_', f_grid_num, '.jpg', sep = '');
  ...
  #f_file_name2 <- paste('shp/3/ppa_2301_rce_3_mean_', order_1, '_', f_grid_num, '.csv', sep = '');
...
rc_ana_4 <- function(f_grid_num, f_breaks_2_num, f_season, f_adj){
  ...
  if(f_season == 2){
    for (f_ii in 1:f_breaks_2_num_2){
      if (is.na(f_data_df_3_mean[f_ii]) || (f_data_df_3_mean[f_ii] <= max(f_data_df_3_mean[f_ii + 1:f_ii + 1 + f_adj], na.rm = TRUE))) {
      } else {
    ...
    for (f_ii in 1:f_breaks_2_num_2){
      if (is.na(f_data_df_3_mean[f_ii]) || (f_data_df_3_mean[f_ii] >= min(f_data_df_3_mean[f_ii + 1:f_ii + 1 + f_adj], na.rm = TRUE))) {
      } else {
        cat('serial number of polygon ',f_grid_num,' is: ',  f_ii, '\n');
        f_RCD_1 <- f_ii;
        break;
...
st_write(grid_2, paste0('shp/3/ppa_2301_rce_s', order_1,'_adj',adj_num,'.shp'))
write.csv(data_df_4v, file = paste0('shp/3/ppa_2301_rce_s',order_1,'_adj',adj_num,'.csv'), row.names = FALSE)
