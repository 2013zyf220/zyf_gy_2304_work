...
buffer_1 <- read.csv('shp/4/2301_cq_water7_buf2.csv') #to_be_set
buffer_2 <- buffer_1$buffer

grid_1a <- shapefile(paste0('shp/4/2301_cq_water7_a8_buf800_3d.shp')); #to_be_set
grid_1b <- spTransform(grid_1a, '+init=epsg:4326');
grid_2 <- st_read(paste0('shp/4/2301_cq_water7_a8_buf800_3d.shp')); #to_be_set

if(lst_data == 1){
  lst_1 <- raster(paste0('raster/ppa_2301_lst_s', order_1,'.tif')); #to_be_set
  dis_1 <- raster(paste0('raster/a2301_disr2b.tif')); #land cover data #to_be_set
}else{
  lst_1 <- raster(paste0('raster/ppa_2301_lstb_s', order_1,'.tif')); #to_be_set
  dis_1 <- raster(paste0('raster/a2301_disr3b.tif')); #land cover data #to_be_set
}
...
jpeg(paste0('shp/4/res1/ppa_2301_rce_lst_v1_', order_1,'.jpg'), width = 800, height = 600, quality = 100)  # Adjust width, height, and quality as needed
plot(lst_2, main = 'Land surface temperature', col = custom_colors);
plot(grid_1b, add = T);
dev.off()  # Close the jpeg device

jpeg(paste0('shp/4/res1/ppa_2301_rce_dis_v1_', order_1,'.jpg'), width = 800, height = 600, quality = 100)  # Adjust width, height, and quality as needed
...
rc_ana_1 <- function(f_grid_num){
  ...
  f_dis_exa <- mask(dis_2,grid_1b[f_grid_num,]);
  f_lst_exa <- mask(lst_2,grid_1b[f_grid_num,]);
  ...
  f_data_df_1 <- list();
  f_data_df_1[1] <- data.frame(dis = f_disv_exa, lst = f_lstv_exa);
  f_data_df_1[2] <- f_dis_exa
  f_data_df_1[3] <- f_lst_exa
...
breaks_2_num <- list();
cir_s <- 1  #to_be_set
cir_e <- grid_end1 #to_be_set
data_df_3_mean_all <- matrix(0, nrow = grid_end1, ncol = 35)
for (ii in cir_s: cir_e){
  breaks_2_num[[ii]] <- round(breaks_2_end[ii] / breaks_2_by[ii]);
  data_df_1[[ii]] <- rc_ana_1(ii)[1];
...
st_write(grid_2, paste0('shp/4/res1/ppa_2301_rce_s', order_1,'_adj',adj_num,'.shp'))
write.csv(data_df_4v, file = paste0('shp/4/res1/ppa_2301_rce_s',order_1,'_adj',adj_num,'.csv'), row.names = FALSE)
write.csv(data_df_3_mean_all, file = paste0('shp/4/res1/ppa_2301_rce_mean_s',order_1,'_adj',adj_num,'.csv'), row.names = FALSE)
#up2023_0924_10:31_e
