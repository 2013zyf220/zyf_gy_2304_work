...
bh_1 <- raster(paste0('chongqing_bh/cq_bh5.tif')); #to_be_set
...
bh_f1 <- function(f_bh_1, f_buffer){
  ...
  f_grid_1 <- shapefile(f_buffer_2); #to_be_set
...
for(c_buf in buffers){
  ...
  c_grid_2$XB1_mean_1 <- res_1_sum[[c_buf]][['bh_6_mean']]
  c_grid_2$XB1_mean_2 <- res_1_sum[[c_buf]][['bh_7_mean']]
  c_grid_2$XB1_std_1 <- res_1_sum[[c_buf]][['bh_6_std']]
  c_grid_2$XB1_std_2 <- res_1_sum[[c_buf]][['bh_7_std']]
  c_grid_2$XB1_ratio <- res_1_sum[[c_buf]][['bh_ratio']]
  ...
  c_bh_data_export[, 1] <- res_1_sum[[c_buf]][['bh_6_len']]
  c_bh_data_export[, 2] <- res_1_sum[[c_buf]][['bh_7_len']]
  c_bh_data_export[, 3] <- res_1_sum[[c_buf]][['bh_6_mean']]
  c_bh_data_export[, 4] <- res_1_sum[[c_buf]][['bh_7_mean']]
  c_bh_data_export[, 5] <- res_1_sum[[c_buf]][['bh_6_std']]
  c_bh_data_export[, 6] <- res_1_sum[[c_buf]][['bh_7_std']]
  c_bh_data_export[, 7] <- res_1_sum[[c_buf]][['bh_ratio']]
  c_bh_data_export[, 8] <- res_1_sum[[c_buf]][['bh_ci']]
  
  write.csv(c_bh_data_export, file = paste0('ppa_2301_k2/test01/shp/6/res2/ppa_2301_bh_1_buf', c_buf, '.csv'), row.names = FALSE)
}
