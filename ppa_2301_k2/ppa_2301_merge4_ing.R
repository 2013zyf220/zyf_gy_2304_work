...
merged_data_f <- function(f_order, f_buffer){
  ...
  f_data_lsi <- read.csv(paste0('3/ppa_2301_lsi_', f_year, '_buf', f_buffer, '.csv')) 
  f_data_bh <- read.csv(paste0('3/ppa_2301_bh_1_buf', f_buffer, '.csv'))
  f_data_svf <- read.csv(paste0('3/ppa_2301_svf_bh_4_buf', f_buffer, '.csv'))
  
  f_data_terrain_1 <- st_read(paste0('3/ppa_2301_terrain_1_buf', f_buffer, '.shp'))
  f_data_terrain_2 <- as.data.frame(f_data_terrain_1)
  f_data_terrain_3 <- f_data_terrain_2[, data_terrain_var]
  f_data_ndvi_1 <- st_read(paste0('3/ppa_2301_ndvi_s', f_order, '_buf', f_buffer, '.shp'))
  ...
  f_data_rotate <- read.csv(paste0("rotate/2301_cq_water_rotate_5.csv"))
  f_data_rw <- st_read(paste0("rotate/2301_cq_water_rw_7.shp"))
  ...
  write.csv(f_merged_data_6, file = paste0('ppa_2301_ana_s', f_order, '_buf', f_buffer, '.csv'), row.names = FALSE)
