...
cat('========================step1.2==========================\n')
#up2024_0520_16:00
#get variables for regression and add elevation
...
dis_1_df <- as.data.frame(dis_1)
colnames(dis_1_df) <- 'dis'
write.csv(dis_1_df, 'dis_1.csv', row.names = FALSE)
...
cat('========================step2.1: summarize data_2 by averaging days==========================\n')
#up2024_0520_16:00
#define function:summarize data_2 by averaging days
...
#==============================================
#up2024_0520_16:00
#basic regression between meteorological variable and distance
...
cat('========================step2.2: export data_2_mean to csv files==========================\n')
#up2024_0520_16:00
#define function: export data_2_mean to csv files

d2_mean_df_1f <- function(f_sub_name, f_vari){
...
#==============================================
#up2024_0520_16:00
#export data_2_mean to csv files

data_2_mean_df <- list() 
for(c_sub_name in subs_name){
  data_2_mean_df[[c_sub_name]] <- list() 
  for(c_vari in varis){
    data_2_mean_df[[c_sub_name]][[c_vari]] <- d2_mean_df_1f(c_sub_name, c_vari)
  }
}

cat('========================step3: add variables for indexes==========================\n')
#up2024_0520_17:00
#add dependent variables for 'index_1'(days merged, streets put together)

for(c_sub_name in subs_name){
  for(c_vari in varis){
    for(ii in times_set){
      c_name <- paste0('mean1_', c_sub_name, '_', c_vari, '_times_', ii)
      index_1[[c_name]] <- data_2_mean[[c_sub_name]][[c_vari]]$r3[,ii]
    }
  }
}
...
#==============================================
#up2024_0520_17:00
#get index_2, add data_2 information by day(using days_ori)

for(c_vari in varis){
  for(ii in times_set){
    for(kk in days_ori){
      c_name_1 <- paste0('ORI_', c_vari, '_time', ii,'_day',kk)
      index_2[[c_name_1]] <- rep(0, len_sites * len_strs_mo)
      for(jj in strs_mo){
        c_1 <- (jj - 1) * len_sites + 1
        c_2 <- jj * len_sites
        index_2[[c_name_1]][c_1:c_2] <- data_2$ORI[[c_vari]][[ii]][[jj]][,kk]
      }
    }
  }
}
...
for(c_sub_name in subs_name){
  ...
  for(c_vari in varis){
    ...
    for(ii in times_set){
      rce_r1[[c_sub_name]][[c_vari]][[ii]] <- rce_1_f(data_2_mean[[c_sub_name]][[c_vari]]$r3[, ii], dis_2)
    }
  }
}
...
fig_1f <- function(f_vari){
  jpeg(paste0('FIG1/fig1_data_2_mean_vs_dis_', f_vari,'.jpg'), width = 1500, height = 1500, res = 300)
  par(mfrow = c(len_subs, len_times_set), oma = c(3, 3, 3, 3), mar = c(2.8,2.8,2.8,2.8),  mgp = c(2, 0.5, 0)) 
  for(c_sub_name in subs_name){
    for(ii in times_set){
      f_y <- data_2_mean[[c_sub_name]][[f_vari]]$r3[,ii]
...
for(c_vari in varis){
  data_2b[[c_vari]] <- list()
  for(ii in times_set){
    data_2b[[c_vari]][[ii]] <- matrix(0, nrow = len_sites * len_strs_mo, ncol = len_days_ori)
    for(jj in strs_mo){
      f_1 <- (jj - 1) * len_sites + 1
      f_2 <- jj * len_sites
      for(kk in days_ori){
        data_2b[[c_vari]][[ii]][f_1:f_2,kk] <- data_2$ORI[[c_vari]][[ii]][[jj]][,kk]
      }
    }
  }
}
...
fig_2f <- function(f_sub, f_vari){
  ...
  for(ii in times_set){
    for(jj in strs_mo){
      ...
      f_2 <- jj * 50
      f_x <- dis_3[[f_sub]]
      f_y <- as.vector(data_2b[[f_vari]][[ii]][f_1:f_2,days_trans_f(f_sub)])
...
fig_3f <- function(f_vari, f_time){
  ...
  for(jj in strs_mo){
    for(kk in days_ori){
      f_1 <- (jj - 1) * 50 + 1
      f_2 <- jj * 50
      f_y <- data_2b[[f_vari]][[f_time]][f_1:f_2,kk]
