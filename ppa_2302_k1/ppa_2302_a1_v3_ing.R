...
times_0_set <- c(1,2,3) #to_be_set_key
...
for(ii in times_0_set){
  ...
  for(jj in strs_co){
    ...
    for(kk in days_set){
      ...
      data_str_1_TP[[ii]][[jj]][[kk]] <- data_str_1[[ii]][[jj]][[kk]]$TP
...
data_str_2_RH_MEAN <- list()
for(ii in times_0_set){
  data_str_2_TP_MEAN[[ii]] <- list()
  data_str_2_RH_MEAN[[ii]] <- list()
  for(jj in strs_mo){
    data_str_2_TP_MEAN[[ii]][[jj]] <- rowMeans(data_str_2_TP_MAT[[ii]][[jj]])
    data_str_2_RH_MEAN[[ii]][[jj]] <- rowMeans(data_str_2_RH_MAT[[ii]][[jj]])
  }
}
