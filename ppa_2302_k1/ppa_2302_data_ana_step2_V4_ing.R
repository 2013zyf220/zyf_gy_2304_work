...
rce_2_f <- function(f_1){
  f_interval <- 3 #to_be_set #changed
  f_1_len <- length(f_1)
...
for(c_vari in varis){
  ...
  for(ii in times_set){
    ...
    for(jj in strs_mo){
      ...
      for(kk in days_ori){
        rce_rb1[[c_vari]][[ii]][[jj]][[kk]] <- rce_2_f(data2_2_ori[[c_vari]][[ii]][c_1: c_2,kk][seq_1]) #changed
      }
    }
  }
}
  ...
  for(c_vari in varis){
    ...
    for(ii in times_set){
      ...
      for(jj in strs_mo){
        c_1 <- (jj - 1) * len_sites + 1
        c_2 <- jj * len_sites
        rce_rb2[[c_sub_name]][[c_vari]][[ii]][[jj]] <- rce_2_f(data2_2_mean[[c_sub_name]][[c_vari]][c_1: c_2,ii][seq_1])
      }
    }
  }
}
  ...
  rce_r1[[c_vari]] <- list()
    for(ii in times_set){
      ...
      for(jj in strs_mo){
        ...
        for(kk in days_ori){
          rce_r1[[c_vari]][[ii]][[jj]][[kk]] <- rce_1_f(data2_2_ori[[c_vari]][[ii]][c_1:c_2,kk][seq_1], dis_1[seq_1]) #changed
      }
    }
  }
}
