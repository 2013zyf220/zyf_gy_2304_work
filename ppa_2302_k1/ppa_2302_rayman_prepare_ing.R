...
paras <- c('rec', 'recw', 'recg') #to_be_set
...
rayman_f1 <- function(f_para_tr, f_para_ws, f_time){
  ...
  rayman_1[,5] <- ele_2
  rayman_1[,6] <- timez_2
  rayman_1[,7] <- tp_data[[f_para_tr]][[f_time]]
  rayman_1[,8] <- tp_data[[f_para_tr]][[f_time]]
  rayman_1[,9] <- tp_data[[f_para_ws]][[f_time]]
  rayman_1[,10] <- could_2
  rayman_1_df <- as.data.frame(rayman_1)
  colnames(rayman_1_df) <- col_names_set
  write.csv(rayman_1_df, paste0('RAYMAN/rayman_1_time', f_para_tr, f_para_ws, ii, '.csv'), row.names = FALSE)
  write.table(rayman_1_df, file = paste0('RAYMAN/rayman_1_time', f_para_tr,  f_para_ws, ii, '.txt'), sep = ' ', row.names = FALSE, col.names = TRUE, quote = FALSE)
...
for(c_para_tr in paras){
    for(c_para_ws in paras){
      for(ii in times_set){
        rayman_f1(c_para_tr, c_para_ws, ii)
    }
  }
}